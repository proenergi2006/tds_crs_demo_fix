  ...
  <div class="p-6 intro-y">
    ...
    <div v-else>
      ...
      <div class="bg-white p-6 rounded shadow grid grid-cols-2 gap-6 text-sm mb-6">
        <div class="space-y-2">
          <div class="grid grid-cols-3"><span class="font-medium text-gray-600">Customer</span>
            <span class="col-span-2">: {{ penawaran.customer.nama_perusahaan }}</span></div>
          <div class="grid grid-cols-3"><span class="font-medium text-gray-600">Cabang</span>
            <span class="col-span-2">: {{ penawaran.cabang.nama_cabang }}</span></div>
          <div class="grid grid-cols-3"><span class="font-medium text-gray-600">Metode</span>
            <span class="col-span-2">: {{ penawaran.metode || '-' }}</span></div>
          <div class="grid grid-cols-3"><span class="font-medium text-gray-600">Ketentuan Order</span>
            <span class="col-span-2">: {{ penawaran.order_method || '-' }}</span></div>
          <div class="grid grid-cols-3"><span class="font-medium text-gray-600">Masa Berlaku</span>
            <span class="col-span-2">: {{ formatDate(penawaran.masa_berlaku) }} - {{ formatDate(penawaran.sampai_dengan) }}</span></div>
          <div class="grid grid-cols-3"><span class="font-medium text-gray-600">Tipe Pembayaran</span>
            <span class="col-span-2">: {{ penawaran.tipe_pembayaran || '-' }}</span></div>
          <div class="grid grid-cols-3"><span class="font-medium text-gray-600">Status</span>
            <span class="col-span-2">: <span class="capitalize text-blue-600">{{ penawaran.status }}</span></span></div>
            <div class="grid grid-cols-3">
        ...
        <div class="space-y-2">
          <div class="grid grid-cols-3"><span class="font-medium text-gray-600">Harga Penawaran</span>
            <span class="col-span-2">: {{ formatCurrency(penawaran.harga_dasar) }}</span></div>
          <div class="grid grid-cols-3"><span class="font-medium text-gray-600">Refund</span>
            <span class="col-span-2">: {{ formatCurrency(penawaran.refund) }} / volume</span></div>
          <div class="grid grid-cols-3"><span class="font-medium text-gray-600">Other Cost</span>
            <span class="col-span-2">: {{ formatCurrency(penawaran.other_cost) }} / volume</span></div>
          <div class="grid grid-cols-3"><span class="font-medium text-gray-600">Diskon</span>
            <span class="col-span-2">: {{ formatCurrency(penawaran.discount) }}</span></div>
          <div class="grid grid-cols-3"><span class="font-medium text-gray-600">OAT</span>
            <span class="col-span-2">: {{ formatCurrency(penawaran.oat) }} / volume</span></div>
          <div class="grid grid-cols-3"><span class="font-medium text-gray-600">Toleransi Penyusutan</span>
            <span class="col-span-2">: {{ penawaran.toleransi_penyusutan }}%</span></div>
            <div class="grid grid-cols-3">
        ...
        <table class="min-w-full text-sm">
          ...
          <tbody>
            <tr v-for="item in penawaran.items" :key="item.id_penawaran_item" class="hover:bg-slate-50">
              ...
              <td class="px-4 py-2 text-right">{{ formatCurrency(penawaran.harga_dasar) }}</td>
              <td class="px-4 py-2 text-right font-medium">{{ formatCurrency(calcJumlah(item)) }}</td>
            </tr>
          </tbody>
          <tfoot class="bg-slate-50">
            <tr><td colspan="3" class="text-right font-semibold px-4 py-2">Subtotal</td>
              <td class="text-right font-semibold px-4 py-2">{{ formatCurrency(subtotal) }}</td></tr>
            <tr><td colspan="3" class="text-right px-4 py-2">Diskon</td>
              <td class="text-right px-4 py-2 text-red-500">-{{ formatCurrency(totalDiskon) }}</td></tr>
            <tr><td colspan="3" class="text-right px-4 py-2">PPN 11%</td>
              <td class="text-right px-4 py-2">{{ formatCurrency(ppn) }}</td></tr>
            <tr><td colspan="3" class="text-right px-4 py-2">Total OAT</td>
              <td class="text-right px-4 py-2">{{ formatCurrency(totalOAT) }}</td></tr>
      ...
      <div class="flex gap-2 mt-6">
        <Button variant="primary" v-if="penawaran.status === 'approved_bm'" @click="verifikasi">Setujui</Button>
        <Button variant="danger" v-if="penawaran.status === 'approved_bm'" @click="tolak">Tolak</Button>
      </div>
    </div>
  </div>
</template>
...
const totalOAT = computed(() => (Number(penawaran.value.oat) || 0) * totalVolume.value)
const grandTotal = computed(() => subtotal.value - totalDiskon.value + ppn.value + totalOAT.value)
